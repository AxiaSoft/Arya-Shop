// ═══════════════════════════════════════════════════════════════
// ADMIN PRODUCTS — GPT‑5 FINAL (بدون Drag & Drop)
// فرم بعد از آپلود تصویر یا رندر دوباره، پاک نمی‌شود
// ═══════════════════════════════════════════════════════════════
            ...
            return `
              <div class="glass rounded-2xl p-5 flex items-center gap-4 animate-fade" style="animation-delay: ${i * 0.05}s">
                ...
                <div class="flex gap-2">
                  <button 
                    onclick="state.editProduct = state.products.find(p => p.id === '${product.id}'); state.productDraft={title:'',category:'',price:0,stock:0,description:'',mainImage:'',gallery:[],original_price:0,_synced:null}; render()"
...
// فقط یک بار از محصول به درفت sync می‌شود
          ...
          <div class="space-y-5">
            ...
            <div>
              ...
              <input 
                ...
                oninput="state.productDraft.title = this.value"
              >
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                ${
                  hasDiscount
                    ? `<p class="text-xs text-emerald-400 mt-1">${discountPercent}% تخفیف</p>`
                    : `<p class="text-xs text-white/40 mt-1">در صورت وارد کردن قیمت اصلی بالاتر، تخفیف محاسبه می‌شود.</p>`
                }
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select 
                  name="category" 
                  class="w-full input-style"
                  onchange="state.productDraft.category = this.value"
                >
                  <option value="">بدون دسته</option>
                  ${state.categories.map(cat => `
                    <option value="${cat.id}" ${d.category === cat.id ? 'selected' : ''}>
                      ${cat.title}
                    </option>
                  `).join('')}
                </select>
            ...
            <div>
              ...
              >
                  ...
                  d.mainImage
                    ? `
                      <div class="w-full max-h-56 rounded-xl overflow-hidden mb-3">
                        <img src="${d.mainImage}" class="w-full h-full object-cover">
                      </div>
                    ...
                    : `
                      ...
                      <p class="text-sm text-white/70 text-center">برای انتخاب تصویر کلیک کنید</p>
                    `
                }
                ...
                <input 
                  ...
                  accept="image/*"
                  class="hidden"
                  onchange="handleMainImageFiles(this.files)"
                >
              </div>
            </div>
            ...
            <div>
              ...
              <div class="flex gap-3 overflow-x-auto pb-2">
                ${
                  d.gallery.length === 0
                    ? `<p class="text-white/40 text-sm">هنوز تصویری اضافه نشده.</p>`
                    : d.gallery.map((img, i) => `
                        <div class="glass rounded-xl p-2 flex-shrink-0 w-32">
                          ...
                          <div class="flex items-center justify-between gap-1">
                            ...
                            <div class="flex gap-1">
                              <button class="btn-ghost px-2 py-1 rounded-lg text-[10px]" onclick="moveGalleryItem(${i}, -1)">◀</button>
                              <button class="btn-ghost px-2 py-1 rounded-lg text-[10px]" onclick="moveGalleryItem(${i}, 1)">▶</button>
                            </div>
                          </div>
                        </div>
            ...
            <div>
              ...
              <textarea 
                ...
                oninput="state.productDraft.description = this.value"
